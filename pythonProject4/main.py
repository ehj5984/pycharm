            ...
            if None != re.search('/block/getBlockData', self.path):
               ...
               # 블록 요청 from에 음수값, 0값 예외 처리
                ...
                try:
                    ...
                    # 인덱스에 해당하는 블록 가져오기
                    # 블록은 3갠데 유저가 입력한 인덱스가 1부터 10일때 => data.append(out of range) 하고 아래 반복문 그대로 써서 블럭정보 가져오기
                    ...
                    elif ( startPoint < 0 or endPoint < 0 ): # startpoint 또는 endpoint 가 음수인 경우 out of range
                        print("out of range")
                        data.append(" out of range ")


                    else: # 범위보다 많은 블록이 있는 경우 start 와 end로 반복문 돌리기
