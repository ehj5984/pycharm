...
class myHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if None != re.search('/api/v1/addrecord/*', self.path):
            ...
            print(pdict) # {encoding:utf-8, lang:ko, loc:seoul} /pdict는 null값
            ...
            if ctype == 'application/json':
                ...
                #  load your str into a dict
                #print(type(tempDict))
                temp = tempDict['이름']+"의 주소는 "+tempDict['주소']+", 이메일은 " + tempDict['이메일']
                self.send_response(200)
                self.send_header('Content-type', 'application/json')
                self.end_headers()
                self.wfile.write(bytes(temp, encoding="utf-8"))
                ...
                #print(postvars.keys())
                #print(type(postvars))
                #print(postvars.values())
                #temp = list(postvars.values())[0]
                #print(postvars.keys()) //프린트찍어가면서 , , ,
